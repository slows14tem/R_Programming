...
df2$대사증후군수 <- rowSums(df2[, 9:13])

head(df2)
#대사증후군 factor로 형변환
df2$성별코드 <- as.factor(df2$성별코드)
...
library(party)
#분류모델 학습 ctree(target ~ 타겟 판별에 영향을 미치는 요소들, data=train)
model2 <- ctree(대사증후군 ~ 성별코드+수축기혈압+이완기혈압+공복혈당+트리글리세라이드+HDL콜레스테롤+허리둘레 , data = train)
